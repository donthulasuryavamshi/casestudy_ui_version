...
public class MentorView 
	...
	private static void init2() {
		...
		public static void view_Training_Status() {
			...
			try{  
	        	 Connection connection=DataBaseConnector.initilizConnection();  
	        	 PreparedStatement stmt=connection.prepareStatement("select batch_id, batch_mentor_id, batch_startDate, batch_endDate from batch where batch_mentor_id=? ");
	        	 stmt.setString(1, MentorModel.getMentorId());
	        	 ResultSet rs=stmt.executeQuery();  
	        	 System.out.println("--------------------");
		     		System.out.println("*****My Lp Batch Status*****");
		     		System.out.println("---------------------");
		     		while(rs.next())
		     		{
		     		System.out.println("Batch Id -:"+rs.getString(1));
		     		System.out.println("Batch Mentor Id -:" +rs.getString(2));
		     		System.out.println("Batch Start Date -:" +rs.getDate(3));
		     		System.out.println("Batch End Date-:"  +rs.getDate(4));
		     		stmt.setString(1, MentorModel.getMentorId());
		     		System.out.println("________________________");
		     		}  
		     		connection.close(); 
		...
		public static void edit_Training_Status() {
			...
			try{  
				...
				Connection connection=DataBaseConnector.initilizConnection();
	        	 ...
	        	 PreparedStatement stmt=connection.prepareStatement("UPDATE Batch SET batch_startDate=?,batch_endDate=?  Where batch_mentor_id=? and batch_id=?");
	        	 ...
	        	 stmt.setString(3, MentorModel.getMentorId());
	        	 ...
	        	 if(rs > 0)
	        	 {
	        		 System.out.println();
	        		 System.out.println();
	        		 System.out.println("Batch Status Updated");
	        		 view_Training_Status();
	        	 }
	        	 else
	        	 {
     		...
     		connection.close(); 
    	 }catch(Exception e){ System.out.println("**********Please Enter valid Details*********");}
			MentorView.training_Calender();
		...
		public static void enroll_Lps() {
				...
				try{  
		        	 Connection connection=DataBaseConnector.initilizConnection(); 
		        	 PreparedStatement stmt=connection.prepareStatement("UPDATE LpDetails SET batch_Id=? Where lp_id=?");
		        	 System.out.println("Enter Lp Id: ");
		        	 String lpId=scanner.next();
		        	 System.out.println("Enter the Batch Id to be enrolled the Lp into : ");
		        	 String batchId=scanner.next();
		        	 stmt.setString(2, lpId);
		        	 ...
		        	 if(rs > 0)
		        	 {
		        		 System.out.println();
		        		 System.out.println();
		        		 System.out.println("__________________Student Sucessfully Enrolled!!!!______________");
		        		 System.out.println();
		        		 System.out.println();
		        	 }
		        	 else
